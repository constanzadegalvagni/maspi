...
dataset_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'BaseOCR_MultiStyle')
...
# declaracion modelo
model = MLP_2hidden(HOG_FEATURE_LENGTH, Nh, Nh, Nc)
loss_fnc = nn.MSELoss()
optimizer = optim.SGD(model.parameters(), lr=1e-1)

nepochs = 200
test_losses = []
train_losses_2hidden=[]
...
for epoch in range(nepochs):
    ...
    val_correct = running_correct / (j-1)
    test_losses.append(val_correct)    
    train_losses_2hidden.append(epoch_loss)
    # print('Epoca: %d - train loss: %f - train correctos: %f - test correctos: %f' % \
    #       (epoch, epoch_loss, epoch_correct,val_correct))
    
plt.plot(np.arange(200), test_losses,label="dos capas")
plt.legend()
plt.title("Test correct")
plt.show()
plt.plot(np.arange(200),train_losses_1hidden,label="una capa")
plt.plot(np.arange(200),train_losses_2hidden,label="dos capas")
plt.legend()
